...
void print_queue(task_t *head);
void calculate_estimation(task_t *old_task);
...
void switch_task() 
	...
	if (current_queue != idle_task) 
		...
		printf("%d \n",tmp->id);
		// give an indication
		printf("child%d's ticks++ \n",current_task->id);
		// check whether the task has been finished
		if(exit_task()&&(current_queue->next !=0))
			{
		...
		printf("Before Sort ");
		unsort_queue ->next = current_queue;
		print_queue(unsort_queue);	
		head->next = current_queue;
		// sort the queues to fit the SPN algorithm
		bubblesort(current_queue);
		printf("After  Sort ");
		print_queue(head);
...
int exit_task() 
	{
	if ((current_task->id)*(current_task->ticks) >= 100) 
		{
		return 1;
		}
	else 
...
void setup_children(int count) 
	...
	unsort_queue = (task_t *)malloc(sizeof(task_t));
